    ...
    UInt                d )
{
    ...
    if ( d == 1 ) {
        ...
        else {  /* now the loop for primes between 2^16 and 2^24 */
      ...
      if (ip > 1 && id > 0 && id <= 24 && ip <= SIZE_LARGEST_INTERNAL_FF)
        {
          id1 = id;
          q = ip;
          while (--id1 > 0 && q <= SIZE_LARGEST_INTERNAL_FF) /* new constant in ffdata.h */
            q *= ip;
          if (q <= SIZE_LARGEST_INTERNAL_FF) /* new constant in ffdata.h */
            {
              /* get the finite field                                                */
